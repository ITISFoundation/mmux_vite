...
def flask_list_function_jobs_for_jobcollectionid():
    ...
    try:
        ...
        jobs = _get_all_items(
            job_api_instance.list_function_jobs,
...
def _create_training_file_from_jobs(
    ...
    df_jobs = pd.DataFrame([_get_job_dict(job) for job in completed_jobs])
...
def flask_manual_uq_propagation_with_uncertainty():
    ...
    try:
        ...
        ## for some reason, much more noisy than Dakota's sampling
        samples = create_manual_uq_samples(
            input_vars_sanitized, distributions, num_samples, seed
        )
        df = pd.DataFrame(samples)
        UQ_SAMPLES_FILE = run_dir / "manual_uq_samples.csv"
        df.to_csv(UQ_SAMPLES_FILE, index=False)
