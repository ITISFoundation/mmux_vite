...
from mmux_flaskapi.utils.webserver_config import OsparcApi, get_osparc_api, OsparcApiException
...
class TestOsparcApiClients:
    ...
    def test_get_job_collection_api_creation(self):
        ...
        with patch.dict(os.environ, self.test_env):
            with patch('mmux_flaskapi.utils.webserver_config.FunctionJobCollectionsApi') as mock_collections:
                with patch('mmux_flaskapi.utils.webserver_config.ApiClient'):
        ...
        with patch.dict(os.environ, self.test_env):
            with patch('mmux_flaskapi.utils.webserver_config.UsersApi') as mock_users_api_class:
                with patch('mmux_flaskapi.utils.webserver_config.ApiClient'):
                    ...
                    mock_users_api = Mock()
                    mock_users_api.get_my_profile.side_effect = OsparcApiException("Connection failed")
                    mock_users_api_class.return_value = mock_users_api
...
class TestGetOsparcApiHelper:
    ...
    def test_get_osparc_api_none_instance(self):
        ...
        with patch('mmux_flaskapi.utils.webserver_config.current_app', mock_app):
            with pytest.raises(ValueError, match="OsparcApi instance is not initialized in the Flask app"):
                get_osparc_api()
    ...
    def test_get_osparc_api_not_connected(self):
        ...
        with patch('mmux_flaskapi.utils.webserver_config.current_app', mock_app):
            with pytest.raises(ValueError, match="OsparcApi instance is not connected to the osparc backend"):
                get_osparc_api()
