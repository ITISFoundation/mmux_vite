...
def create_sample_jobs():
    ...
    for i in range(20):
        ...
        job = {
            ...
            "outputs": {"Efficiency_Percent": efficiency, "PowerConsumption_W": power_consumption},
        }
        jobs.append(job)
...
def example_basic_usage():
    ...
    preprocessor.setup_sign_switching(
        ...
        output_sign_switches=[
            "Efficiency_Percent"
        ],  # Example: maximize efficiency (minimize negative)
    )
...
def example_integration_with_training_workflow():
    ...
    try:
        ...
        original_results = load_and_inverse_transform_results(
            results=simulated_results, config_file_path=config_file
        )
...
def example_separated_workflow():
    ...
    print(
        f"   Temperature_C range: {df['Temperature_C'].min():.2f} to {df['Temperature_C'].max():.2f}"
    )
    ...
    print(
        f"   x1 (Temperature_C z_score) range: {df_normalized['x1'].min():.2f} to {df_normalized['x1'].max():.2f}"
    )
    print(
        f"   y2 (PowerConsumption_W min_max) range: {df_normalized['y2'].min():.2f} to {df_normalized['y2'].max():.2f}"
    )
    ...
    preprocessor3.setup_sign_switching(
        input_sign_switches=["FlowRate_LperMin"], output_sign_switches=["Efficiency_Percent"]
    )
    df_switched = preprocessor3.fit_transform(df)
    print(
        f"   FlowRate_LperMin range: {df['FlowRate_LperMin'].min():.2f} to {df['FlowRate_LperMin'].max():.2f}"
    )
    print(f"   x3 (switched) range: {df_switched['x3'].min():.2f} to {df_switched['x3'].max():.2f}")
    print(
        f"   Efficiency_Percent range: {df['Efficiency_Percent'].min():.2f} to {df['Efficiency_Percent'].max():.2f}"
    )
    ...
    print(
        f"   y1 (Efficiency sign-switched): {df_both['y1'].min():.2f} to {df_both['y1'].max():.2f}"
    )
    print(
        f"   y2 (PowerConsumption min_max): {df_both['y2'].min():.2f} to {df_both['y2'].max():.2f}"
    )
...
def example_advanced_features():
    ...
    preprocessor.setup_normalization(
        ...
        output_normalizations={"output_to_maximize": "min_max", "output_to_minimize": "z_score"},
    )
    preprocessor.setup_sign_switching(
    ...
    for var in input_vars:
        ...
        transformed_range = (
            df_transformed[config.mapped_name].max() - df_transformed[config.mapped_name].min()
        )
        print(
            f"  {var} ({config.normalization_method}): {original_range:.3f} -> {transformed_range:.3f}"
        )
    ...
    preprocessor_edge.setup_variables(
        input_vars=["constant_var"], output_vars=["output_to_maximize"]
    )
    preprocessor_edge.setup_normalization(input_normalizations={"constant_var": "z_score"})

    try:
        input_data: pd.DataFrame = df_edge[["constant_var", "output_to_maximize"]]  # type: ignore
        _ = preprocessor_edge.fit_transform(input_data)
