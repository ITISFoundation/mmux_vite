...
def create_sample_data():
    ...
    for i in range(50):
        ...
        efficiency_percent = (
            0.7
            + 0.2 * (temperature_c - 60) / 40
            + 0.1 * (pressure_bar - 5) / 5
...
def test_basic_filtering():
    ...
    filtered_preprocessor.filter_by_names(
        input_names=selected_inputs, output_names=selected_outputs, exclude=False
    )
    ...
    filtered_preprocessor2.filter_by_names(
        input_names=exclude_inputs, output_names=exclude_outputs, exclude=True
    )
...
def test_pattern_filtering():
    ...
    input_patterns = [
        ...
        r"Flow.*",
        r".*Ratio",
    ]  # Variables ending with _C or _V, containing Flow, or ending with Ratio
    output_patterns = [r".*_W$"]  # Variables ending with _W

    preprocessor.filter_by_patterns(input_patterns=input_patterns, output_patterns=output_patterns)
...
def test_predicate_filtering():
    ...
    output_normalizations = {"Efficiency_Percent": "z_score", "PowerConsumption_W": "z_score"}

    preprocessor.setup_normalization(input_normalizations, output_normalizations)

    print("Variables with normalization:")
    ...
    preprocessor2.filter_variables(
        input_predicate=long_name_predicate, output_predicate=long_name_predicate
    )
...
def test_combined_filtering():
    ...
    df_jobs = pd.DataFrame([{**job["inputs"], **job["outputs"]} for job in jobs[:10]])
...
def test_copy_and_filter():
    ...
    print("Base preprocessor variables:")
    ...
    temp_flow_preprocessor = create_filtered_preprocessor(
        base_preprocessor, input_patterns=[r".*Temp.*", r".*Flow.*"], output_patterns=[r".*Flow.*"]
    )
    ...
    print(
        f"  Outputs ({len(efficiency_power_vars['outputs'])}): {efficiency_power_vars['outputs']}"
    )
