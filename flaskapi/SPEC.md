...
api: all dakota request models carry per-variable `input_log_scales`/`output_log_scales` (dict[str,bool]; camelCase `inputLogScales`/`outputLogScales`) → log-space training + inverse on response (V16, V41qz)
--- env ---
...
V16: log-scale per-variable flag (from request payload) reaches Dakota preprocessing → sample/train in log space, inverse on response (⊥ train linear when UI=log) — distilled, ported (§T9, node/SPEC.md V12); output log-scale positivity guarded by V41qz
...
V39qz: log-scale Dakota response paths (`sumo_along_axes` + `sumo_grid_evaluation`) ! inverse-transform point estimates and uncertainty together, preserving original variable names; for log output y=exp(z), returned `std` ~= `std_z*|y|`, ⊥ raw/log-space values or missing mapped-name keys (B25qz)
V40qz: endpoint tests that assert remote-only oSPARC list results ! see persistent `local_job_store` data; each test/app fixture starts with an isolated empty store or explicitly scopes local fixtures, so local-merge behavior is tested separately (B26qz)
V41qz: `validate_output_log_scale_positivity` (dakota_models.py) rejects any `output_log_scales[var]=True` (raises ValueError → 422) unless every completed job's output for that var is strictly >0 (log undefined for ≤0); mirrors the uniform-input `min>0` guard on `DistributionParams.log_scale` (V16). Code comment previously mislabeled this "V34" — V34 is the unrelated lhs.py method-dispatch invariant (B27qz)
...
T9|x|PORT [topic=fullstack-logscale] accept per-variable log-scale flag in dakota request models → preprocess sample/train in log space, inverse on response; tests|V16, V41qz, ../node/SPEC.md V12
...
B25qz|2026-08-07|log-scale `sumo_along_axes`/`sumo_grid_evaluation` response paths did not consistently inverse-transform mapped output points and `_std` values; regression tests exposed a missing mapped input key and raw/log-space results instead of original-space `{10,20,40}` with delta-method std `{1,4,6}`|V39qz
B26qz|2026-08-07|universal local-store merge (V15/V31) read persistent `runs_local` artifacts during remote-list endpoint tests; 9 list tests returned extra local functions/jobs/collections (up to 250 jobs) instead of the fixture's empty or mocked remote result|V40qz
B27qz|2026-08-07|`validate_output_log_scale_positivity` (dakota_models.py) docstring mislabeled its invariant as "V34"; V34 is the unrelated lhs.py method-dispatch invariant, so the new positive-positivity guard was undocumented under a wrong §V number — corrected to V41qz|V41qz
