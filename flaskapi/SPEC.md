...
api: POST `/compute_sobol_indices` `{output,inputVars[],distributions,numSamples,FunctionJobs[],seed(≥0)}` → `{sobol:{inputVar:{main,total}}, sobolSecondOrder:{varA:{varB:float}}}` — scipy-based Saltelli sampling + `scipy.stats.sobol_indices`, always-on second-order (#470, §T20,T23)
--- env ---
