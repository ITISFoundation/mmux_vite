...
api: POST `/compute_sobol_indices` `{output,inputVars[],distributions,numSamples,FunctionJobs[],seed,secondOrder?}` → `{sobol:{inputVar:{main,total}}, sobolSecondOrder?:{varA:{varB:float}}}` (#470; `secondOrder` opt-in second-order/pairwise indices, §T23)
--- env ---
