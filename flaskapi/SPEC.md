...
api: POST `/compute_sobol_indices` `{output,inputVars[],distributions,numSamples,FunctionJobs[],seed(≥1)}` → `{sobol:{inputVar:{main,total}}}` — Dakota native `variance_based_decomp` (#470); scipy migration + always-on `sobolSecondOrder` planned, ⊥ yet implemented (§T20,T23, drift B18)
--- env ---
...
V34 (PLANNED, ⊥ yet implemented — B18): once migrated to scipy-based sampling, `SobolIndicesRequest.seed` may accept `0` (numpy/scipy RNGs accept 0, unlike Dakota's NIDR parser); current code still requires `seed ≥ 1` (NIDR constraint, B17) until V32 lands
V35 (PLANNED, ⊥ yet implemented — B18): Sobol' second-order (pairwise interaction) indices are to be ALWAYS computed alongside first/total (V32 scipy-based approach, same batch); `SobolIndicesRequest` currently has no `secondOrder` opt-in flag (never added); response is to always include `sobolSecondOrder:{varA:{varB:float}}` symmetric ∀ unordered pairs, no self-pair (diagonal filled on frontend from first-order index); formula: closed-form Jansen/Saltelli estimator reusing the same f_A/f_B/f_AB sample evaluations (Saltelli et al. 2010, analytically derived + to be validated vs scipy's own Ishigami unit tests, see R1)

## §R
R1: Ishigami test function (3-param) published analytical Sobol' indices (Saltelli 2007/2008), to validate the PLANNED scipy migration (V32/V35) against once implemented: first-order S1≈0.314, S2≈0.442, S3≈0; total-order S1_total≈0.558, S2_total≈0.442, S3_total≈0.244; second-order S_12≈0, S_13≈0.244, S_23≈0. ⊥ yet backed by a test — `test_sobol_indices_ishigami_analytical` to be added once V32/V35 land (B18)
...
T20|.|Sobol' sensitivity indices migration (#470, #485): replace current Dakota `variance_based_decomp`/`parse_sobol_indices_output` path in `evaluate_sobol_indices` w/ scipy-based computation — Saltelli A/B/AB sampling locally honouring distributions, single `evaluate_sumo()` batch, `scipy.stats.sobol_indices` for first/total order; tests grounded vs Ishigami analytical reference (R1)|V32,B18,../node/SPEC.md T22
...
T23|.|second-order (pairwise) Sobol' indices for `/compute_sobol_indices` (#470 follow-up, pairs T20): closed-form Jansen/Saltelli estimator reusing T20's f_A/f_B/f_AB arrays (no extra surrogate evaluations); always returned (⊥ opt-in), response `sobolSecondOrder:{varA:{varB:float}}` symmetric ∀ pairs; validate vs Ishigami refs (R1)|V32,V35,B18,../node/SPEC.md T22
