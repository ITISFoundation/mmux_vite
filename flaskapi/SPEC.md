...
api: POST `/sumo_cross_validation` `{inputVars[],output,FunctionJobs[]}` → `{observed[],predicted[],predictedStd[]}`
...
api planned: same response + `{sobolOrderContributions:{firstOrder,secondOrder,thirdAndHigher,firstOrderCiLow,firstOrderCiHigh,secondOrderCiLow,secondOrderCiHigh,thirdAndHigherCiLow,thirdAndHigherCiHigh,heuristicNoiseFloor}}`; `firstOrder=M1`, `secondOrder=M2`, `thirdAndHigher=R`
method note: grouped post-hoc estimate for variable set `G`: `S_T,G = 1 - S_closed,complement(G)`; under order-2 truncation `≈ Σ_{i∈G}S_i + Σ_{i<j, i∈G∨j∈G}S_ij`; exact only when omitted order `≥3` terms negligible; single nominal-point UQ with complement fixed ≠ global Sobol group index when cross-group interaction exists
method note: `S_T,G` measures expected variance from varying `G` across distribution of complement, not variance from one conditional propagation at arbitrary nominal complement values
...
V46jk: every dakota response uses fixed schema fields; a QoI/input identifier ! be interpolated into a top-level response key, ⊥ force clients to reproduce serializer casing
...
V42qa: ∀ varying-input count `d∈[4,25]`, `sobolSecondOrder[i][j]` ! use a validated estimator/design that identifies each unique pair term; ⊥ infer `O(d²)` pair terms only from `d` first/total values or claim complementary single-column matrices suffice without proof; exact design cost/variance tradeoff documented (pair-specific mixed evaluations may cost `O(d²)`); validate additive, pair-interaction, Ishigami, and ≥10-dimensional analytic benchmarks w/ tolerance scaled by bootstrap uncertainty (closes B26nc)
V43pt: order masses use unique ANOVA terms: `M1=Σ_i S_i`; `M2=Σ_{i<j} S_ij` (unordered pair once); `R=1-M1-M2`; ∴ `M1+M2+R=1` by construction while `Σ_i S_Ti` generally ≠1 because interactions repeat across totals; ⊥ clamp raw finite-sample estimates before aggregation
V44vw: one shared-row bootstrap resample ! recompute all `S_i`,`S_Ti`,`S_ij`, then `M1`,`M2`,`R` per replicate → percentile CIs preserving estimator covariance; `R` noise assessment reports CI-vs-zero as primary plus `heuristicNoiseFloor=median` individual first/total CI half-width as explicitly rough comparator; ⊥ label `R` practically small/large without user/domain threshold
V45xy: `R` = closure residual after unique first/second-order masses, not independently observed higher-order variance; `M1+M2+R=1` arithmetic identity always, scientific interpretation requires unbiased/validated `S_i`/`S_ij` and independent-input ANOVA assumptions; `Σ_iS_Ti` and group total indices ⊥ expected to close at 1
...
T30|x|PR #509 Copilot review: fix B25 — drop the internal `sanitize_varnames`/`sanitize_varname`/`sanitize_varnames_dict` call on `input_vars`/`distributions` at the top of `evaluate_sobol_indices` (funs_evaluate.py), removed now-unused imports; regression test w/ a sanitize-affecting var name (`"sigma blood (kg)"`, space+parens) asserting the response is keyed by the exact original name|V41,B25
T31rb|.|replace underdetermined second-order formula with validated arbitrary-`d` pair estimator/design (target 4–25; choose/document cost vs accuracy, including pair-specific mixed evaluations if required); jointly bootstrap pair indices + `M1/M2/R`; extend response model/route tests and analytic numerical regressions|V42qa,V43pt,V44vw,V45xy,B26nc,../node/SPEC.md T21gh
...
B26nc|2026-08-17|current `S_ij=((S_Ti-S_i)+(S_Tj-S_j)-Σ_{k≠i,j}(S_Tk-S_k))/2` infers every pair from only `d` total-minus-first values; system underdetermined for `d≥4` (sum of reported pairs collapses to 0 at `d=4`, can become negative for larger `d`) → pair indices and derived order masses scientifically invalid for typical 4–25 parameter studies|V42qa,V43pt
B27jk|2026-09-22|`sumo_cross_validation` emitted QoI-derived top-level keys without a response model; global camelCase serialization changed user identifiers and forced the frontend to guess response keys|V46jk
