...
V32: `flask_download_job_collection_csv`'s remote branch ! reject a job collection whose `job_ids` is empty with a 422 (cannot infer function schema), ⊥ default `function_uid` to `""` and let it fail deep inside `_function_schema_vars` (closes B16)
...
B15|2026-07-08|copilot review #490: merging `origin/develop` into this stack branch silently dropped `get_osparc_api_if_connected` (function body + `__all__` entry) from `webserver_config.py` during manual conflict resolution — no conflict markers left, `osparc.py`/tests still imported it → `ImportError` at app import time, entire flaskapi suite (506 tests) failed to collect. Also left a stale test (`test_get_osparc_api_not_connected`) asserting a connectivity check in `get_osparc_api()` that V31 explicitly forbids — removed|V31
B16|2026-07-08|copilot review #490: `flask_download_job_collection_csv`'s remote branch fell back to `function_uid=""` when a job collection's `job_ids` is empty (no jobs to read a `function_uid` from) → `_function_schema_vars("")` call fails deep inside CSV serialization instead of a clear error at the endpoint boundary|V32
