...
V35: ∀ SuMo plot step → QoI selector reachable in the plot header (mmux-testid `qoi-select`), ⊥ SuMo-only exclusion; changing `selectedQoI` must re-fetch + re-render the plotted QoI (#499), so users are never stuck on the initially-selected output after entering the validation view
