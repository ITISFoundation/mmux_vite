...
svc: `mock-osparc` (e2e-only) → in-backend oSPARC test-double: `create_flask_app()` injects it as `app.osparc_api` when `is_test_environment()` (`OSPARC_API_BASE_URL` contains `test`); deterministic data (≥5 SUCCESS jobs, LHS seed 42, valid `sumo_cross_validation` inputs/outputs → flow ⊥ skip) lazy-imported from `tests/e2e/mock_osparc/` (on `PYTHONPATH`); ⊥ real oSPARC HTTP, ⊥ prod code path. (chosen over standalone HTTP stub: avoids re-impl of oSPARC SDK schema surface)
...
env (e2e): `SERVICE_MODE=SUMO` ∧ `PERMISSIONS=READ-ONLY` ∧ `DEPLOYMENT_MODE=LOCAL` ∧ `OSPARC_API_BASE_URL`=test sentinel (∴ `is_test_environment` → in-backend test-double)
...
V11: backend-under-e2e ⊥ reach real oSPARC — `OSPARC_API_BASE_URL`=test sentinel (`is_test_environment` true → `create_flask_app` injects in-backend test-double as `app.osparc_api`); ⊥ `api.osparc.io`/`api.sim4life.io` HTTP in e2e
...
T8|.|e2e tooling: add `@playwright/test` (node/ devDep) + `node/playwright.config.ts` (webServer array, baseURL `:8888`, viewport 1600×900, `snapshotPathTemplate`→`tests/e2e/__snapshots__/`, `maxDiffPixelRatio` threshold, single pinned chromium project) + `tests/e2e/` TS layout + `npm run test:e2e` + `make test-e2e`|V10,V12,node/SPEC.md T9
T9|.|`mock-osparc` in-backend test-double: `create_flask_app()` injects `MockOsparcApi` as `app.osparc_api` when `is_test_environment()`; duck-types `get_{functions,job,job_collection,users}_api()` surface used by `blueprints/osparc.py`; deterministic data module `tests/e2e/mock_osparc/` (≥5 SUCCESS jobs, LHS seed 42, valid `sumo_cross_validation` inputs/outputs) lazy-imported via `PYTHONPATH`, ⊥ prod path|V11
T10|.|e2e stack launch (via playwright `webServer`): live Flask (`OSPARC_API_BASE_URL`=test sentinel, `SERVICE_MODE=SUMO`, `PERMISSIONS=READ-ONLY`, `DEPLOYMENT_MODE=LOCAL`, `PYTHONPATH`+=`tests/e2e`) + web (vite dev/preview) w/ `/flask/*` proxy split → app origin (`PLAYWRIGHT_BASE_URL`, default vite-proxy port; Caddy `:8888` path = CI/docker)|V1,V11
T11|.|SuMo read-only e2e spec (port behavior from `test/playwright-automation:tests/e2e/test_sumo_local.py`): reset persistence (POST `/flask/text-file`) → assert deployment SUMO/READ-ONLY → `select-function-btn-{uid}` → fill `input-block-Min/Max` → `next-button` → wait `jobs-loading` + "Creating AI model…" hidden → assert `sumo-validation-view`/`qoi-select`/`.js-plotly-plot`/`MAE:`/`RMSE:`/`extend-sampling-btn` disabled; ADD `toHaveScreenshot` @Setup grid + validation view (mask/seed Plotly); console-error guard. add missing testids on current branch (`jobs-loading`,`sumo-validation-view`,`select-function-btn-{uid}`) per branch component diffs|V10,V13
T12|.|CI: run `make test-e2e` in pinned Playwright docker image; commit baselines; gate merge on green pixel diff|V12,§C
