...
svc: `mock-osparc` (e2e-only) → standalone HTTP stub serving oSPARC API shapes (from `flaskapi/tests/osparc_api_mocks.py`), deterministic (≥5 SUCCESS jobs, LHS seed 42 → flow ⊥ skip); backend `OSPARC_API_BASE_URL` → this svc
...
env (e2e): `SERVICE_MODE=SUMO` ∧ `PERMISSIONS=READ-ONLY` ∧ `DEPLOYMENT_MODE=LOCAL` ∧ `OSPARC_API_BASE_URL`→`mock-osparc` (∴ `is_test_mode`)
...
V11: backend-under-e2e ⊥ reach real oSPARC — `OSPARC_API_BASE_URL`→`mock-osparc` (`is_test_mode` true); ⊥ `api.osparc.io`/`api.sim4life.io` in e2e
...
T9|.|`mock-osparc` svc: standalone HTTP stub (`tests/e2e/mock_osparc/`) serving oSPARC fn/jobs/collection/sampling endpoints from `flaskapi/tests/osparc_api_mocks.py` shapes; deterministic seed (≥5 SUCCESS jobs, LHS seed 42, valid `sumo_cross_validation`) so SuMo flow ⊥ `skip`|V11
T10|.|e2e stack launch (via playwright `webServer`): live Flask (`OSPARC_API_BASE_URL`→`mock-osparc`, `SERVICE_MODE=SUMO`, `PERMISSIONS=READ-ONLY`, `DEPLOYMENT_MODE=LOCAL`) + web (vite preview/Caddy) w/ `/flask/*` split → app on `:8888`|V1,V11
