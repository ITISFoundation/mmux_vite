...
V36zn: dev `mmux-vite-backend` runs as host UID/GID with `./flaskapi:/app` bind-mounted → `UV_CACHE_DIR` ! point below writable `/app` (e.g. `/app/.cache/uv`), ⊥ uv default `/.cache/uv` (B22zn)
V37kp: dev `mmux-vite-backend` live source bind `./flaskapi:/app` ! preserve the image-built `/app/.venv` via a named volume mounted at `/app/.venv`; startup ⊥ install/download locked dependencies (B23kp)
V38hp: dev backend uses the root-seeded `/app/.venv` as read-only: `entrypoint.sh` ! invoke Flask through `uv run --no-sync`; numeric identity logging ! use `id` directly because host UID/GID may lack a passwd entry in the image (B24hp)
V39dw: Dakota intermediate runs ! resolve from explicit `DAKOTA_RUNS_DIR` or the Flask project directory, ⊥ `Path.cwd()` parent traversal; the directory ! be created with parents so the non-root dev backend never writes below `/` (B26dw)
V40as: sampling intermediate runs ! resolve from explicit `SAMPLING_RUNS_DIR` or the Flask project directory, ⊥ `Path.cwd()` parent traversal; the directory ! be created with parents so the non-root dev backend never writes below `/` (B27as)
...
B22zn|2026-09-21|dev `mmux-vite-backend` correctly ran as the host UID/GID (V31vr) but did not override uv's cache location; uv defaulted to `/.cache/uv`, which the non-root process cannot create, so every `make run-develop-*` backend exited before Flask started with permission denied|V36zn
B23kp|2026-09-21|dev `./flaskapi:/app` bind mount hid the image-built `/app/.venv`; `uv run` recreated it at every backend start and downloaded locked packages, so transient container DNS failure prevented Flask from booting|V37kp
B24hp|2026-09-21|the named `/app/.venv` volume retained image-built dependencies, but Docker seeded it as root; default `uv run` then synchronized the project and tried to rewrite package metadata as the host UID, failing with permission denied before Flask started; the same unmapped UID made `whoami` logging fail noisily|V38hp
B25sd|2026-09-21|`make build` could not pull Docker Hub base `node:20-alpine` because the registry TLS handshake timed out before any application build step ran|no §V (external transient registry/network failure)
B26dw|2026-09-21|Dakota blueprint import set its run directory through `Path.cwd().parent.parent.parent`; container cwd `/app` collapsed this traversal to `/runs_dakota`, so the non-root dev backend failed during import before Flask could start|V39dw
B27as|2026-09-21|after the Dakota cwd fix, sampling blueprint import exposed the identical `Path.cwd().parent.parent.parent` error: container cwd `/app` resolved its storage to unwritable `/runs_sampling`, preventing Flask startup|V40as
