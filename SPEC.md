...
T28|.|DOCS-MIGRATION 2/4: add `.github/workflows/docs.yml` — on push to `main` touching `docs/**`, build+deploy via `mkdocs gh-deploy --force` to `gh-pages` branch; enable Pages (Settings→Pages→branch `gh-pages`) — replaces manual `make gh-deploy` so docs-as-we-go ships automatically, no separate "remember to deploy" step|T27
T29|.|DOCS-MIGRATION 3/4: port old repo's Makefile targets (`devenv`,`serve`,`build`,`gh-deploy`→now redundant w/ T28 but keep for local preview) into this repo's root `Makefile` as `docs-serve`/`docs-build` etc., pointed at `docs/`|T27
T30|.|DOCS-MIGRATION 4/4: archive `ZurichMedTech/model-intelligence` (GitHub Settings→Archive repository) + replace its README with a short redirect note pointing at this repo's `docs/`; ⊥ delete — keeps old `zurichmedtech.github.io/model-intelligence` Pages URL alive so existing external links (e.g. issue #450) don't gain a *new* 404, just a stale-but-live page|T27,T28
