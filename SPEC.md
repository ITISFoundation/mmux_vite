...
V26: development compose `mmux-vite-web` healthcheck `start_period` ≥ 120s when startup runs mounted-source dependency installation before Vite; ⊥ health-gate `mmux-vite-app` during install and produce no-upstream 503 (B8)
...
B8|2026-08-07|development `mmux-vite-web` inherited the production image healthcheck with a 20s start-period, but its compose override mounted `node/` and ran `npm install` before Vite; `mmux-vite-app` correctly waited for `service_healthy`, then aborted when web became unhealthy and Caddy logged `no upstreams available` 503|V26
