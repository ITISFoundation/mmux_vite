...
T22|.|SuMo validation statistical rigor: paired t-test (prediction bias) + convergence analysis (accuracy vs sample size) in cross-validation flow|flaskapi/SPEC.md T22,node/SPEC.md T20
T23|.|sensitivity/correlation indices from UQ Monte Carlo samples, single multi-param view (#470, beyond 3-param 1D/2D/3D limit)|flaskapi/SPEC.md T23,node/SPEC.md T21
...
T27|.|DOCS-MIGRATION 2/4: add `.github/workflows/docs.yml` — on push to `main` touching `docs/**`, build+deploy via `mkdocs gh-deploy --force` to `gh-pages` branch; enable Pages (Settings→Pages→branch `gh-pages`) — replaces manual `make gh-deploy` so docs-as-we-go ships automatically, no separate "remember to deploy" step|T26
T28|.|DOCS-MIGRATION 3/4: port old repo's Makefile targets (`devenv`,`serve`,`build`,`gh-deploy`→now redundant w/ T27 but keep for local preview) into this repo's root `Makefile` as `docs-serve`/`docs-build` etc., pointed at `docs/`|T26
T29|.|DOCS-MIGRATION 4/4: archive `ZurichMedTech/model-intelligence` (GitHub Settings→Archive repository) + replace its README with a short redirect note pointing at this repo's `docs/`; ⊥ delete — keeps old `zurichmedtech.github.io/model-intelligence` Pages URL alive so existing external links (e.g. issue #450) don't gain a *new* 404, just a stale-but-live page|T26,T27
