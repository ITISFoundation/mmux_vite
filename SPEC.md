...
V34: ∀ host → Manual & Tutorial links point to the single canonical ZMT GitHub Pages docs URL, ⊥ host-derived `manual.<host>` subdomain (404, #450); the link target is NOT built from the detected host
V35: ∀ SuMo plot step → QoI selector reachable in the plot header (unique `mmux-testid` `sumo-plot-qoi-select`), ⊥ SuMo-only exclusion; changing `selectedQoI` must re-fetch + re-render the plotted QoI (#499), so users are never stuck on the initially-selected output after entering the validation view
V36: ∀ surrogate plot fetch → request key includes the plotted axis' sampling range [min,max] (buildDakotaRequestKey.axisRanges), ⊥ range-blind key; changing an input's sampling range must invalidate the cache + re-fetch (#501), so the 1D/2D/3D curve reflects an expanded input range
V37qn: ∀ interactive control targeted by an e2e selector → use a unique semantic `mmux-testid` for that workflow control, ⊥ `.first()`/`.last()`/`.nth()` positional selection among duplicate test ids
...
T12|x|CI: run `make test-e2e` in pinned Playwright docker image (`mcr.microsoft.com/playwright:v1.62.1-noble`, tag==`@playwright/test`); baselines generated/committed via `make test-e2e-update-docker` + verifiable locally via `make test-e2e-docker`, both in the SAME image (font-stable, §V12); `e2e-tests` job gates merge on green pixel diff vs committed baselines|V12,§C
...
B19mu|2026-08-28|dependabot PR #551 (`@mui/material` 7.3.2→9.3.1) failed node-tests on TWO independent causes: (a) `npm ci` → ERESOLVE because sibling `@mui/icons-material@^7` peers `@mui/material@^7` — bumping one MUI MAJOR alone breaks peer resolution; (b) after a lockstep icons-material bump, `tsc -b` → 104 errors — MUI v9 is a breaking MAJOR (renamed icon exports e.g. `InfoOutline`→`InfoOutlined`/`HelpOutline`→`HelpOutlined`; removed `TextField` `InputProps`→`slotProps.input`; tightened `Typography` `component` overload + removed `fontFamily` prop). A major MUI bump is a migration, not a version bump dependabot can perform alone|V33mu
...
B20|2026-08-23|#450 `node/src/components/navigation/TutorialManualLinks.tsx` built `https://manual.${simplifiedHost}/...` for osparc hosts too, but `manual.<osparcHost>` (e.g. manual.osparc-staging.io) does not exist → 404 "link to manual does not work"; manuals & tutorials now live on ZMT GitHub Pages, so the link must be a single canonical docs URL rather than a host-derived subdomain|V34
B21|2026-08-24|#499 SuMo plot steps consume the global `selectedQoI` but `node/src/components/plots/SuMoPlotsSteps.tsx` only rendered the QoI selector for UQ/MOGA; the stepped "back" button cannot return to OutputSetup, so once in the SuMo validation view a user had no way to change the QoI → plot stayed on the initially selected output (reported as "reload shows the wrong/stale QoI")|V35
B22|2026-08-24|#501 `node/src/utils/dakotaRequestKey.ts` `buildDakotaRequestKey` did not encode the plotted axis' sampling range, and `node/src/components/plots/Curves1DPlot.tsx` effect deps omitted `distribution`; so widening an input's range (e.g. h in the "dT Nerve vs h" plot) produced an identical request key → V16 dedup blocked the refetch and the 1D curve x-range stayed stale (reported as "1D plot does not reflect expanded h sampling range")|V36
B23|2026-08-28|#560 review: `tests/e2e/sumo-readonly.spec.ts`/`sumo-qoi-reload.spec.ts` scoped the `qoi-select` locator to `[mmux-testid="sumo-validation-view"]`, but the QoI selector was correctly moved to the plot `Header` (outside the validation view) in the SuMo plot workflow — the scoped locator never matched, so both specs would fail once run against the real DOM|V13
B24|2026-09-16|#560 CI: SuMo e2e specs used a page-wide strict `qoi-select` locator after the plot-header selector was added, but OutputSetup and the active plot header both rendered that test id, so Playwright failed with a strict-mode violation before exercising the workflow|V35
B25hz|2026-09-16|#560 follow-up: OutputSetup and the SuMo plot header intentionally rendered different QoI controls under the same `qoi-select` id, so `.last()` made e2e behavior depend on DOM order|V37qn
