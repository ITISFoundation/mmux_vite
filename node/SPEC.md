...
consumes: POST `/flask/dakota/compute_sobol_indices` → parameter first/total + pairwise second-order plots; planned order-mass summary `{M1,M2,R}` + CIs/noise comparator (§T21gh)
method note: post-hoc group approximation uses returned unique `S_i`/`S_ij`; `S_T,G ≈ Σ_{i∈G}S_i + Σ_{i<j, i∈G∨j∈G}S_ij` only under negligible order-`≥3` contribution; nominal-point propagation with other inputs fixed does not equal global group Sobol total order under interaction
...
V16: Dakota plot fetches (1D/2D/3D) deduped by stable logical request key {axes,sliderValues,QoI,fn,jobList,logScale,axisRanges}; same key → ⊥ new fetch (INV-006, §T5)
...
V36jg: ∀ Dakota plot fetch (Curves1D/Surface2D/IsoSurface3D) triggered by effect dependency change ⊥ have overlapping in-flight requests; cancel previous request via AbortController before starting new one (B22jg)
V37kp: ∀ completed Dakota plot fetch response ! be applied unless response arrives from the in-flight request matching the current requestKey; ignore/discard stale responses to prevent old data overwriting fresh plots (B22jg)
V38rq: `buildAxisRanges(distribution, axes)` ! include NaN endpoint values in the serialized key; non-uniform distributions compute endpoints as `mean ± 2.5σ` which may be NaN if distribution state is incomplete; exclude incomplete ranges so key changes track finite-only range updates (B23mu)
...
T32jg|.|fix B22jg: add AbortController to Curves1DPlot/Surface2DPlot/IsoSurface3DPlot; cancel previous request before starting new one when effect dependencies change; test race scenario (manual wait + rapid dependency changes → fresh data never overwritten)|V36jg,V37kp
T33mu|.|fix B23mu: `buildAxisRanges()` filters out NaN endpoints (check both `isFinite(range.min) && isFinite(range.max)`) so incomplete distributions don't pollute keys; non-uniform ranges with missing mean/std are excluded from serialization, preventing stale-key caching when state loops back|V38rq
...
B22jg|2026-09-23|PR #599 adds `distribution` to plot component effect dependencies, triggering new fetches when range changes; overlapping in-flight requests both update `lastFetchedKey` on completion, so if older fetch completes last it overwrites fresh data and leaves cache pointing at stale key. Example: fetch A(K1) starts, then fetch B(K2) starts before A completes; if B finishes then A, A's response restores old plot + sets `lastFetchedKey=K1`; next request for K2 sees mismatch and starts unnecessary fetch|V36jg,V37kp
B23mu|2026-09-23|`buildAxisRanges()` treats `NaN` as a finite endpoint (checks only `min !== undefined && max !== undefined`); non-uniform distributions in PlotTools.tsx compute range as `mean ± 2.5*std`, which yields `[NaN, NaN]` if distribution state is incomplete (missing mean/std). Serializing `[NaN,NaN]` into the key and later serializing finite values results in different key → refresh occurs; but then `[NaN,NaN]` → `[null,null]` in JSON, making key appear consistent when state loops back to incomplete, skipping needed refresh cycles|V38rq
