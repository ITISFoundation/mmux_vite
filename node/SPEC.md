...
V28rs: ∀ `toHaveScreenshot` comparisons → `maxDiffPixelRatio` = 0; any differing pixel ! green e2e
...
B20qf|2026-08-18|`node/playwright.config.ts` allowed `maxDiffPixelRatio` = 0.01, so visible Stats button changed pixels stayed below threshold and stale baseline passed|V28rs
