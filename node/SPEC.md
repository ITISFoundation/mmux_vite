...
T8|x|PORT [topic=fullstack-logscale] `utils/distributionDiagnostics.ts` + per-variable log-scale UI (InputVariableDist/OutputVariableDist) + log-scale plot rendering (Curves1D/Surface2D/IsoSurface3D) end-to-end per V12; fix B24 (`IsoSurface3DPlot` now also sends `outputLogScales` for the QoI, matching Curves1D/Surface2D). No backend present in this worktree (flaskapi V16/T9 not merged here) so the FE→BE contract is exercised only up to the request payload, not verified against a live Dakota response|V12,B24, ../flaskapi/SPEC.md V16,T9
...
T24|.|PORT-CANDIDATE [from `DO-NOT-MERGE-feature/local-functions`, untracked until now]: generalize `fetchRetry.ts` terminal-error short-circuit from `status===404`-only to any non-429 4xx (`isTerminalClientError`) so client errors stop pointless retries; absent on `main`; draw from branch impl|-
T25|.|PORT-CANDIDATE [from `DO-NOT-MERGE-feature/local-functions`, untracked until now]: null-safety guards in `GridSearchSampling.tsx`/`LHSSampling.tsx` for undefined `selectedFunctionUid`/`selectedDistribution` (dependency-array fixes) — prevents crash when sampling panel renders before a function/distribution is chosen; absent on `main`; draw from branch impl|-
...
B24|2026-07-04|T8 verification: `IsoSurface3DPlot` computed `inputLogScales` and sent them to `/flask/dakota/sumo_grid_evaluation`, but never sent `outputLogScales` for the selected QoI (only used for input axis rendering) despite its own comment claiming "the output flag is still sent to the backend" — the code never matched the comment. Curves1D/Surface2D already sent it correctly. Backend would train/invert the isosurface QoI in linear space even when the user enabled log-scale for it via OutputVariableDist, silently diverging from the other two plot types for the same QoI. Fixed by adding `outputLogScales` to the request body and folding it into the dedup key, mirroring the other two components|V12
