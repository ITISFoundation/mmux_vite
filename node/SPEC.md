...
T18|.|add e2e mock function (`tests/e2e/mock_osparc/data.py`) with underscore-bearing variable names (e.g. `sigma_blood`) exercising validation/1D-2D-3D-plot/UQ-propagation request paths, so pixel-snapshot suite would have caught B18-class bugs; current fixture's `x1..x4`/`y..y4` names are underscore-free so `snakeToCamelCase` is a no-op and masks this bug class|V24,B18
