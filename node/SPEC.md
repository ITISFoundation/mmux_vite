...
B23wq|2026-09-23|PR #600 (`jgo/fix-validation-qoi-499`, sharing the validation QoI selector between SuMo/MOGA) branched before B22ab/V30ab landed on develop; its `SuMoValidation.tsx` commit still read `data[validationQoI]`/`data[\`${validationQoI}Hat\`]` instead of the fixed `{observed,predicted}` contract, so rebasing the PR onto develop produced a real content conflict (not just noise) between the stale QoI-keyed extraction and `getValidationSeries`. Resolved by keeping `getValidationSeries` (V30ab) and layering the PR's `validationQoIOverride`-prop/rename refactor on top|V30ab
