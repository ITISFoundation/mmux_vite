...
V25: UQ view surfaces per-input sensitivity to the selected QoI beyond the 3-var 1D/2D/3D plot limit via two single-plot views covering ALL input vars at once: `CorrelationIndicesPlot` (Pearson+Spearman bar chart, backend `../flaskapi/SPEC.md V28`) + `SobolIndicesPlot` (Main+Total effect bar chart, backend `evaluate_sobol_indices`/`../flaskapi/SPEC.md V32` PLANNED scipy migration); both render as a flat vertical stack in `UQ.tsx` today, ⊥ yet a stepper (§T22 pending)
