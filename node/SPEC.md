...
V26: every public `UploadJobCollectionButton` prop contract change ! update all JSX callers + focused tests in the same change; `npm run build` and `UploadJobCollectionButton.test.tsx` ! fail from missing/removed props or missing default button label (B20qz)
V27: `useAutoDetectQoiScale` fires `sumo_cross_validation` twice per candidate QoI (`outputLogScales[qoi]=false` then `true`), compares RMSE in original units, applies the winner as an UNTOUCHED DEFAULT only; once a user manually toggles a QoI's scale (`outputLogScaleUserSet[uid][qoi]`), that QoI locks and the hook never overrides it again, even as the job-set grows (V12). Eligibility gate: ≥5 completed jobs with numeric output AND every output >0 (mirrors ../flaskapi V41qz + distributionDiagnostics min>0). Cached by (uid, QoI, sorted job-uid list) so an unchanged job-set never re-fires (INV-006)
V40: per-input-variable `distributionUserModified` flag (FunctionContext, keyed by uid then var) — true once a user manually edits a variable's distribution entry (vs auto-inferred/refreshed); orthogonal to `distribution` (always the source sent to backend); gates existing-mode CSV-upload merge + drives the blue "user-modified" marker (B32)
...
T8|x|PORT [topic=fullstack-logscale] `utils/distributionDiagnostics.ts` + per-variable log-scale UI (InputVariableDist/OutputVariableDist) + log-scale plot rendering (Curves1D/Surface2D/IsoSurface3D) end-to-end per V12|V12, ../flaskapi/SPEC.md V16,V41qz,T9
...
B20qz|2026-08-07|`UploadJobCollectionButton` changed its public props (`buttonLabel` required; `existingFunctions` removed) without updating existing `FunctionList` callers and focused tests; build failed with 7 TS errors and all 4 component tests could not find expected default `Upload Data` button|V26
B28|2026-08-07|`inferDistributionType` (distributionDiagnostics.ts) rounds inferred uniform min/max to 3 significant digits, rounding outward so bounds still cover the observed data; code comment previously cited phantom `V34`|V27
B32|2026-08-07|per-variable "user-modified" flag (`distributionUserModified`) is orthogonal to `distribution` (manual edits set true, auto-inference/refresh clears); drives the blue marker + gates existing-mode CSV merge; code comments cited phantom `B32/V40` pair|V40
B33|2026-08-07|log scale is a per-variable orthogonal linear/log sampling `scale` (`global.d.ts` `InputVarSelection`; log-uniform for uniform shape, log-normal for normal), replacing the old per-type `logScale`/`log-normal` split; LHS sampling screen shows a warning (display-only, not wired into sampling bounds), `logSpaceToLinearMoments` converts log→linear moments for intuitive entry, and OutputVariableDist shows a derived log-normal note; code comments cited phantom `B33/V40`|V12,V40
