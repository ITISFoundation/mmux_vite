...
export const InputVariableDist = () => {
  const { selectedFunction, inputVars, distribution, setDistribution } =
    useFunctionContext();
  ...
  const ConstantInputDistribution = ({ inputVar }: { inputVar: string }) => {
    return (
      <InputBlock
        name="Value"
        value={
          localDistribution[inputVar].value !== undefined
            ? localDistribution[inputVar].value
            : NaN
        }
    ...
    return (
      <>
        <InputBlock
          ...
          value={
            localDistribution[inputVar].mean !== undefined
              ? localDistribution[inputVar].mean
              : 0.0
          }
          onChange={(value) =>
            handleSetValue(inputVar, "mean", value as number)
          }
        />
        <InputBlock
          ...
          // TODO remove default values; just for development speed
          value={
            localDistribution[inputVar].std !== undefined
              ? localDistribution[inputVar].std
              : 1.0
          }
    ...
    return (
      <>
        <InputBlock
          ...
          value={
            localDistribution[inputVar].min !== undefined
              ? localDistribution[inputVar].min
              : NaN
          }
        ...
        <InputBlock
          ...
          value={
            localDistribution[inputVar].max !== undefined
              ? localDistribution[inputVar].max
              : NaN
          }
    ...
    return (
      <>
        <InputBlock
          name="Log Location"
          value={
            localDistribution[inputVar].location !== undefined
              ? localDistribution[inputVar].location
              : NaN
          }
          onChange={(value) =>
            handleSetValue(inputVar, "location", value as number)
          }
        />
        <InputBlock
          name="Log Scale"
          value={
            localDistribution[inputVar].scale !== undefined
              ? localDistribution[inputVar].scale
              : NaN
          }
          onChange={(value) =>
            handleSetValue(inputVar, "scale", value as number)
          }
        />
      </>
    );
  ...
  const setInitialValues = (
    ...
    if (serviceMode === "SUMO") {
      if (["angle", "anglewidth"].includes(inputVar)) {
        ...
        ["gap", "length", "interelectrodespacing"].includes(inputVar)
      ) {
        return { distribution: "uniform", min: 0.2, max: 2 };
      } else if (
        ["silicone_extra", "siliconeextra", "siliconepadding"].includes(
          inputVar
        )
      ) {
    ...
    // Tissue Properties Demo
      ...
      if (
        ["sigma_conn", "sigmaconnectivetissue"].includes(inputVar) ||
        ["sigma_interst", "sigmainterstitial"].includes(inputVar)
      ) {
        return { distribution: "normal", mean: 0.08, std: 0.016 };
      } else if (
        ["sigma_fasc_lon", "sigmafasciclelongitudinal"].includes(inputVar)
      ) {
        return { distribution: "normal", mean: 0.57, std: 0.114 };
      } else if (
        ["sigma_fasc_tra", "sigmafascicletransversal"].includes(inputVar)
      ) {
            ...
            >
              ...
              >
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        handleDistributionChange(
                          inputVar,
                          e.target.value as distribution
                        )
                      }
                    >
                      ...
                      <MenuItem value="log-normal" disabled={true}>
                        LogNormal
                      </MenuItem>
                      <MenuItem value="exponential" disabled={true}>
                        Exponential
                      </MenuItem>
                    </Select>
                  </InputLabel>
                ...
                <>
                  ...
                  ) : localDistribution[inputVar]?.distribution ===
                    "uniform" ? (
