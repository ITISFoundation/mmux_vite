...
export function InputVariableDist() {
  ...
  function ConstantInputDistribution({ inputVar }: { inputVar: string }) {
    return (
      <InputBlock
        name="Value"
  ...
  function NormalInputDistribution({ inputVar }: { inputVar: string }) {
    return (
      <>
        <InputBlock
          name="Mean"
  ...
  function UniformInputDistribution({ inputVar }: { inputVar: string }) {
    return (
      <>
        <InputBlock
          name="Min"
  ...
  function LogNormalInputDistribution({ inputVar }: { inputVar: string }) {
    return (
      <>
        <InputBlock
          name="Log Location"
  ...
  const setInitialValues = (inputVar: string, serviceMode: string): VarSelection => {
    ...
    if (serviceMode === "SUMO" || serviceMode === "MOGA") {
      ...
      if (["gap", "length", "interelectrodespacing"].includes(inputVar)) {
        return { distribution: "uniform", min: 0.2, max: 2 };
      }
      if (["silicone_extra", "siliconeextra", "siliconepadding"].includes(inputVar)) {
    ...
    // Tissue Properties Demo
      ...
      if (["sigma_fasc_lon", "sigmafasciclelongitudinal"].includes(inputVar)) {
        return { distribution: "normal", mean: 0.57, std: 0.114 };
      }
      if (["sigma_fasc_tra", "sigmafascicletransversal"].includes(inputVar)) {
        return { distribution: "normal", mean: 0.16, std: 0.032 };
      }
      if (["sigma_nerve", "sigmanerve"].includes(inputVar)) {
        return { distribution: "normal", mean: 0.34, std: 0.068 };
      }
      if (["sigma_blood", "sigmablood"].includes(inputVar)) {
        return { distribution: "normal", mean: 0.662, std: 0.13 };
      }
      if (["sigma_saline", "sigmasaline"].includes(inputVar)) {
