...
```typescript
import { createConfiguration, SolversApi } from '';
import type { SolversApiListJobsPaginatedRequest } from '';

const configuration = createConfiguration();
const apiInstance = new SolversApi(configuration);

const request: SolversApiListJobsPaginatedRequest = {
...
const data = await apiInstance.listJobsPaginated(request);
console.log('API called successfully. Returned data:', data);
```
